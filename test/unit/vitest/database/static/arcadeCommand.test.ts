...
import { arcadeCommand } from '@/database/static/arcadeCommand.js';
import {
  TEST_COMMAND_INPUT,
...
const ENDPOINT = TEST_DATABASE_CONTEXT.endpoints.command.toString();

describe('arcadeCommand()', async () => {
  ...
  it(`should make a properly formatted POST request to ${ENDPOINT}`, async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      TEST_DATABASE_CONTEXT.endpoints.command,
      {
        method: 'POST',
  ...
  it('should handle a 200 response by returning the result property from the json response', async () => {
    ...
    // Act
    const result = await arcadeCommand(
      TEST_DATABASE_CONTEXT,
      QueryLanguages.SQL,
      TEST_COMMAND_INPUT
    );
